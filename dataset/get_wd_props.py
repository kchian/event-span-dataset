...
def collect_event_items(json_path: Path, candidate_qids: Set, out_path: Path):
    ...
    for prefix, event, value in ijson.parse(bz2.open(json_path)):
        ...
        if (prefix, event) == ("item", "end_map"):
            ...
            if (prefix, event) == ("item.claims", "map_key"):
            ...
            elif prefix == f"item.claims.{property_id}.item.mainsnak.datavalue" and event == "map_key":
                # keep track of value and type for each datavalue
                item_properties[property_id][-1][value] = {}
                map_key = ""

            elif prefix == f"item.claims.{property_id}.item.mainsnak.datavalue.value" and event == "map_key":
                map_key = value

            elif prefix == f"item.claims.{property_id}.item.mainsnak.datavalue.value.{map_key}":
                ...
                item_properties[property_id][-1]["value"] = {"type": str(event), "value": str(value)}

            elif prefix == f"item.claims.{property_id}.item.mainsnak.datavalue.type" and event == "map_key":
                map_key = value

            elif prefix == f"item.claims.{property_id}.item.mainsnak.datavalue.type.{map_key}":
                ...
                item_properties[property_id][-1]["type"] = {"type": str(event), "value": str(value)}

            elif (prefix, event) == ("item.claims", "end_map"):
                for _property_id, _property_values in item_properties.items():
                    wiki_items += [{"qid": qid, "pid": _property_id, "datavalue": _property_values}]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="collect event items from Wikidata dump")
