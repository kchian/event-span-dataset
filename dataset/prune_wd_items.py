...
from utils import load_wd2wikipedia_links
from utils import load_wd_properties, load_wd_descriptions, load_exclusion_wd_props
from utils import load_wiki_event_descriptions
        ...
        for prop in item2wd_props[row["QID"]]:
            if prop in non_event_props:
                ...
                if "*" in disallowed_parent_qids:
                    ...
                    assert prop == "P31"
                    parent_qids = [x["value"]["id"]["value"] for x in item2wd_props[row["QID"]][prop]]
...
def skip_wd_classes(wiki_links, item2wd_props: Dict):
    ...
    for _, row in tqdm(wiki_links.iterrows()):
        ...
        if INSTANCE_OF in wd_item_props:
            parent_items = [x["value"]["id"]["value"] for x in wd_item_props[INSTANCE_OF]]
...
def skip_non_english_wiki(wiki_links, item2wiki_desc: Dict):
    ...
    logging.info("skipping items with no associated en-wiki article or missing label/description")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        ...
        datefmt="%Y-%m-%d %H:%M:%S",
        level=logging.INFO,
        handlers=[logging.StreamHandler(), logging.FileHandler("log_prune_wd_items.txt")],
    )

    parser = argparse.ArgumentParser(description="prune WD items")
    parser.add_argument("wd2wikipedia_links", type=Path, help="Wikidata to Wikipedia links")
    parser.add_argument("wd_item_prop", type=Path, help="Wikidata properties")
    parser.add_argument("wd_item_desc", type=Path, help="Wikidata labels and descriptions")
    parser.add_argument("wiki_event_desc", type=Path, help="Wikipedia event descriptions")
    parser.add_argument("skip_props", type=Path, help="Wikidata props to exclude")
    parser.add_argument("out", type=Path, help="path to write pruned Wikidata to Wikipedia links")
    parser.add_argument("--skipped-out", type=Path, default=None, help="path to write skipped links")
