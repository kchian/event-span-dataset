    ...
    with open(wiki_file_path, "r", errors="ignore", encoding="utf-8") as rf:
        for line in rf:
            ...
            if (lang, title) in wiki2item:
                ...
                first_para = re.sub(r"&lt;a href=\"([^\"]*)\"&gt;([^&]*)&lt;/a&gt;", r"\2", first_para)
...
def write_out(data: List, out_path: Path):
    ...
    with open(out_path, "w") as wf:
        wf.write(f"Wikidata Item\tWikipedia Language\tWikipedia Title\tWikipedia Description\n")
        for row in data:
            wf.write("\t".join(row))
            wf.write("\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="collect Wikipedia descriptions for event articles")
