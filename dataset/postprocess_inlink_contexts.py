...
from utils import load_xlel_tsv, load_wiki_event_descriptions
...
def collect_mentions(wiki_inlinks: pd.DataFrame, item2desc: Dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in tqdm(wiki_inlinks.iterrows()):
        ...
        foundNum = False
        label_numeric_values = set(re.findall(r"[0-9]+", item2desc[row["Wikidata Item"]]["en"]["label"]))
        desc_numeric_values = set(re.findall(r"[0-9]+", item2desc[row["Wikidata Item"]]["en"]["description"]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("wiki_event_desc", type=Path, help="path to Wikipedia Event descriptions")
